        ...
        if (err) {
            ...
            if (doc) {
                ...
                if (doc.admin == 1) {
                    ...
                    res.send('<script>alert("Please Verify Your Account to login"); window.location.replace("'+process.env.hostedlink+'");</script>');
                    return;
                    res.render('login/index');
                }
                ...
                res.send('<script>alert("Invalid Credentials"); window.location.replace("'+process.env.hostedlink+'");</script>');
                return;
                res.redirect('/');
            }
    ...
    try {
        ...
        User.findOne({ email: gmail }, (err, doc) => {
            if (err)
                ...
                if (doc) {
                    res.send('<script> alert("Already Have an account");window.location.replace("'+process.env.hostedlink+'signup"); </script>');
                    return;
                    isSigningUp = false;
                    res.redirect('/signup');
                }
                                    ...
                                    await newUser.save()
                                        .then(data => {
                                            ...
                                            axios.post(encodeURI(hostlink)).then((_) => {
                                                res.send('<script> alert("Email is sent to your walchand college id ' + gmail + '. Please Verify it.");window.location.replace("'+process.env.hostedlink+'"); </script>');
                                                return;
                                            }).catch((err) => {
                                                console.log(err);
                                                res.send('<script> alert("Email has not been sent , Please Verify from admin.");window.location.replace("'+process.env.hostedlink+'"); </script>');
                                                return;
                                            })
                                        })
                                        .catch(err => {
    ...
    return User.findOne({ $and: [{ _id: id }] }, { _id: "1" })
        .then(result => {
            ...
            if (result) {
                ...
                res.send('<script> alert("Invalid link"); window.location.replace("'+process.env.hostedlink+'");</script>');
                return;
...
app.post('/info', (req, res) => {
    ...
    User.findOne({ username: curUsername }, (err, doc) => {
        if (err) {
            ...
            if (doc) {
                res.send('<script> alert("This username already exists"); window.location.replace("'+process.env.hostedlink+'");</script>');
        ...
        if (err)
            ...
            if (doc) {
                ...
                User.findOneAndUpdate({ username: req.body.username }, { password: sha256(String(randNumber)) }, { new: true, useFindAndModify: false }
                    ...
                    var hostlink = process.env.maillink + "?email=" + data.email + "&subject=Change Password for WCE SPACE Account&body=" + mailText;

                    await axios.post(encodeURI(hostlink)).then((_) => {
                        res.send('<script> alert("Email has been sent to your Walchand email id containing temporary password.");window.location.replace("'+process.env.hostedlink+'"); </script>');
                        return;
                    }).catch((err) => {
                        console.log(err);
                        res.send('<script> alert("something is worng please try again later.");window.location.replace("'+process.env.hostedlink+'"); </script>');
                        return;
                    })
                }).catch(err => {
                    console.log("Error in updating" + err);
                    res.send('<script>alert("Please Enter Correct username");window.location.replace("'+process.env.hostedlink+'cpass"); </script>');
                });

                // res.redirect('/');
            }
            else {
                res.send('<script> alert("No such user is available"); window.location.replace("'+process.env.hostedlink+'forget");</script>');
...
app.post('/cpass', (req, res) => {
    ...
    try {
        ...
        User.findOneAndUpdate({ _id: verifyUser._id, password: curPassword }, { password: newPassword }, { new: true, useFindAndModify: false }
            ...
            res.send('<script> alert("Your Password Has been updated");window.location.replace("'+process.env.hostedlink+'home"); </script>');
        }).catch(err => {
            console.log("Error in updating" + err);
            res.send('<script>alert("Please Enter Correct Current Password");window.location.replace("'+process.env.hostedlink+'cpass"); </script>');
        });
        // })
        // .catch(err => {
        //     res.send('<script>alert("Please Enter Correct Current Password");window.location.replace("'+process.env.hostedlink+'cpass"); </script>');
...
app.post('/contribute', async (req, res) => {
    ...
    const contents1 = new Buffer.from(req.files.myFile1.data).toString('base64');
    const contents2 = new Buffer.from(req.files.myFile2.data).toString('base64');
    var postData1 = {
    ...
    var postData2 = {
        name: req.files.myFile2.name,
        mimitype: req.files.myFile2.mimetype,
        img: contents2
    };

    await axios.post(encodeURI(process.env.drivelink), postData1).then(async (fileresp1) => {
        await axios.post(encodeURI(process.env.drivelink), postData2).then((fileresp2) => {
            if (fileresp1.data && fileresp2.data) {
                if (booksstore.branch === "CSE") {
                    sampleCSE = new CSE({
                        ...
                        imgUrl: fileresp2.data.link,
                        bookUrl: fileresp1.data.link,
                    });
                    sampleCSE.save();
                } else {
                            ...
                            if (booksstore.branch === "MECH") {
                                ...
                                if (booksstore.branch === "ELE") {
                                    ...
                                    if (booksstore.branch === "ELET") {
                                        ...
                                        res.send('<script>alert("You didnt selected any branch Please Try again!!!");window.location.replace("'+process.env.hostedlink+'contribute"); </script>');
                                        return;
                ...
                res.send('<script>alert("Thank You for Contributing us");window.location.replace("'+process.env.hostedlink+'contribute"); </script>');
            }
        }).catch((err) => {
            res.send('<script>alert("Error in Uploading File!! please try again");window.location.replace("'+process.env.hostedlink+'"); </script>');
        })
    }).catch((err) => {
        res.send('<script>alert("Error in Uploading File!! please try again");window.location.replace("'+process.env.hostedlink+'"); </script>');
    })
